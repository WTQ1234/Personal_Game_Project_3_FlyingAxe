...
{
    public static void ShowDialog<T>(string defaultDestinationPath, string defaultAssetName, Action<T, string> onScritpableObjectCreated = null)
        where T : ScriptableObject
    {
        var selector = new ScriptableObjectSelector<T>(defaultDestinationPath, defaultAssetName, onScritpableObjectCreated);
    ...
    {
        private Action<T, string> onScritpableObjectCreated;
        private string defaultDestinationPath;
        private string defaultAssetName;

        public ScriptableObjectSelector(string defaultDestinationPath, string defaultAssetName, Action<T, string> onScritpableObjectCreated = null)
        {
        ...
        private void ShowSaveFileDialog(IEnumerable<Type> selection)
        {
            ...
            dest = EditorUtility.SaveFilePanel("Save object as", dest, defaultAssetName, "asset");
            ...
            if (!string.IsNullOrEmpty(dest) && PathUtilities.TryMakeRelative(Path.GetDirectoryName(Application.dataPath), dest, out dest))
            {
                ...
                {
                    this.onScritpableObjectCreated(obj, fileName);
                }
            }
            else
            {
