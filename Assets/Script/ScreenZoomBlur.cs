...
//using OverlayFilters.Demo;

public class ScreenZoomBlur : MonoBehaviour
{
    SortingGroup group;
    int defaultOrder;
    ...
    void Update()
    {
        ...
        {
            if(isFullScreen == false && targetAlpha > 0.1f)
            {
        ...
        {
            if (Input.GetMouseButton(0) && isFullScreen && ((float) Input.mousePosition.x / (float) Screen.width < 0.8f || (float)Input.mousePosition.y / (float)Screen.height < 0.8f))
            {
    ...
    {
        return targetAlpha * currentFade;
    }
